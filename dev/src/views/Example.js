...
export default class Example {
	...
	set example(ex) {
		...
		if (ex) {
			...
			str += "<code class='expression'><svg class='icon load'><use xlink:href='#load'><title>正規表現を読み込む</title></use></svg>" + Utils.htmlSafe(exp) + "</code>";
			if (txt && regex) {
				...
				str += "<hr><code class='text'><svg class='icon load'><use xlink:href='#load'><title>テキストを読み込む</title></use></svg>" + s + (over?"<i>\u2026</i>" : "") + "</code>";
			}
		}
