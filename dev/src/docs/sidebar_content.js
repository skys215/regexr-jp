...
home.kids = [
	...
	label: "我的模式",
	id:"favorites",
	desc: "上面的列表将显示您创建或收藏的所有模式。"+
		"<p>要编辑您创建的模式，请单击其URL或在列表中双击它以加载它，然后打开“保存/共享”进行编辑和保存。</p>",
	search: true,
	kids: []
	},
	...
	label: "模式社区",
	id: "community",
	desc: "欢迎使用模式社区，这是一个由您这样的用户提交的可搜索的模式数据库。"+
		"<p>选择模式后，单击其URL或在列表中双击它以加载完整模式。或使用右箭头图标仅加载表达式或文本。</p>"+
		"<p>通过为模式打分，并通过菜单中的<b>搜索&分享</b>提交您自己的模式来帮助社区变得更好。</p>",
	search: true,
	kids: []
	},
	...
	kids: [
		...
		desc:"RegExr提供了实时可视化结果，语法突出显示，工具提示和撤消/重做（{{getCtrlKey（）}}  -  Z / Y），因此探索正则表达式非常的简单有趣。"+
			"<p>浏览 <b>RegEx参考</b>，测试不同模式的匹配结果，并从<b>模式社区</b>中查询示例。</p>"+
			"<p>您还可以<b>保存</b>您的模式以供日后参考, 或与他人分享。<b>登陆</b>以确保您不会丢失您的模式。</p>"+
			"<p>通过<b>模式设置</b>修改您的模式的详细信息, 分享到<b>社区</b>, 将其设置为私有或者删除。</p>",
		kids: [
			{
			label:"表达式面板",
			...
			kids: [
				...
				label:"替换",
				id: "replace",
				desc:"<b>替换</b>工具用指定的字符串或者模式来替换匹配项。"+
			...
			{
			label:"菜单",
			desc:"<b>菜单</b>包括<b>帮助</b>, <b>RegEx 参考</b>, <b>备忘表</b>, 和<b>模式设置</b>等功能."+
				"<p>双击<b>RegEx 参考</b>中的选定项，将其插入<b>表达式</b>。单击示例旁边的箭头以加载。</p>"+
				"<p>菜单同时包含可搜索的<b>模式社区</b>, 以及您在<b>我的模式</b>中创建或收藏的模式。</p>"
			}
		]
		},
		{
		label:"登陆",
		id: "signin",
		desc:"登陆之前, RegExr创建一个依赖于浏览器cookie的临时帐户。这意味着您无法在其他计算机上访问您的模式，并且您可能会丢失模式如果您的cookie被删除或者过期。"+
			"<p>登录会创建一个永久帐户，因此您可以随时随地访问您的模式。</p>"+
			"<p>收藏夹将自动分配您已有的模式到新账户中。</p>"+
		...
		kids: [
			...
			{
			label:"PCRE (PHP)",
			desc:"PHP {{getPHPVersion()}} 和 PCRE {{getPCREVersion()}} 在我们的服务器端被用于执行您的模式。"
			}
		]
		},
		{
		label:"支持字符串查询",
		desc:"除了内置的<b>保存</b>机制，RegExr还可以创建可共享的链接。RegExr还支持通过查询字符串进行模式预填充。"+
			"<p>字符串查询参数:<ul>"+
			"<li><code>引擎</code> - RegEx引擎(<code>js</code> 或 <code>pcre</code>)</li>"+
			"<li><code>表达式</code> - 填充表达式区域. 建议使用元字符传递完整表达式(<code>/.*/ig</code>)，而不仅仅是模式(<code>.*</code>)</li>"+
